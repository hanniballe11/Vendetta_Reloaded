...
void Building::repair(double amount)
{
    if(amount>0){
            if(integrity+amount>integrity_max){integrity=integrity_max;}
            else{integrity+=amount;}}
    if(!is_constructed && integrity==integrity_max){is_constructed=!is_constructed;}
}

void Building::takeDamage(double damage)
{
    if(damage>0){
            if(integrity-damage<0){integrity=0;}
            else{integrity-=damage;}}
}
...
double Building::getIntegrity() const{return integrity;}
sf::Shape& Building::getIntegrityBar(){return integrity_bar;}
void Building::setIntegrity(double value){
...
double Building::getMaxIntegrity() const{return integrity_max;}
